...
def create_bagit_txt(bag_dir):
    ...
    bagit_version = ".".join(pkg_resources.get_distribution("bagit").version.split(".")[:2])  # Get BagIt version, only MAJOR.MINOR otherwise it will throw error
...
def create_bag_info(bag_dir):
    ...
    total_files = sum(1 for root, dirs, files in os.walk(bag_dir) for file in files) # Count the number of files in the bag
