...
def convert_vob_to_output(input_file, output_file, output_format):
    ...
    if duration_output == 'N/A' or float(duration_output) < 5:
        print(f"[bold orange]Skipping:[/bold orange] {os.path.basename(input_file)} (duration: {duration_output})")
        return None
    ...
    for video_ts_path in video_ts_paths:
        ...
        if vob_files:
            ...
            for vob_file in vob_files:
                ...
                output_file = os.path.join(subfolder, f"{os.path.splitext(vob_file)[0]}.{output_format}")
            ...
            if converted_files:
                if len(converted_files) == 1:
                    ...
                    print(f"[bold green]Moved single {output_format.upper()} file:[/bold green] {final_output}")
                else:
                    # Merge multiple converted files
                    merged_output = os.path.join(subfolder, f"{os.path.basename(output_folder)}.{output_format}")
                    ...
                    try:
                        ...
                        print(f"Error merging {output_format.upper()} files: {e.stderr}")
